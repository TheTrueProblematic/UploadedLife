...
- `index.html` is the single entry document. It loads `Pages/main.html` inside an iframe and attaches `styles.css` and `scripts.js`. The iframe approach keeps scenario navigation contained while allowing the parent window to manage viewport heuristics and audio.
- `Pages/` contains 31 lightweight HTML shells. Each meaningful scenario file sets `data-page="scenario"` plus a `data-scenario` identifier so that shared JavaScript can pull the correct builder. Utility pages (`main`, `mobile`, `nohappiness`, three `identitytheft*`, and `learnmore`) mark their `data-page` values for bespoke rendering logic.
- `scripts.js` defines a single global controller, `UploadedLifeHost`, which handles navigation, state mutation, modal dialogs, responsive fallbacks, and audio mediation. The same script runs both in the parent window and inside every iframe to guarantee consistent behavior.
- `styles.css` centralizes the retro terminal aesthetic: monospace typography, dark palette, grid-based HUD, modal overlay styles, scrollbar hints, and responsive clamps. It also styles the iframe boundary to occupy the entire viewport.
...
7. **Audio Themes**: The soundtrack switches between `base`, `peak`, `spicy`, and `somber` themes depending on the current iframe target. Because modern browsers block autoplay, the host registers pointer and key listeners to unlock audio once the player interacts with any frame or the parent window.
...
- **Administrative Pages**: `main`, `mobile`, three `identitytheft*`, `learnmore`, and `nohappiness` are non-scenario frames rendered through bespoke host logic.

Each HTML shell in `Pages/` contains minimal markup so that the JavaScript host can rebuild the body at runtime. This approach keeps Git history readable while allowing the scenario graph to evolve in one place.

## Asset Inventory
- `index.html`: Parent document with iframe container and `<noscript>` warning.
- `Pages/`: 31 scenario or utility frames named for their identifiers.
...
- `UploadedLifeHost` generates a single looping `<audio>` element in the top window to prevent multiple layers of music as the iframe swaps pages.
...
- The entire experience assumes a widescreen viewport. The parent window measures the width-to-height ratio; if it drops below 1.5, the iframe swaps to `Pages/mobile.html`, which displays instructions to rotate or resize before resuming play.
...
2. Launch any static file server (examples):
   ...
   - `npx serve .`
3. Visit `http://localhost:4173/` (or equivalent). Loading `index.html` via direct file URLs is discouraged because some browsers block iframe-to-parent communication and audio unlock logic in `file://` contexts.
4. To test on smaller screens, resize the browser until the mobile notice appears, then press “I have a wider screen now” to rerun the viewport heuristic.
...
- **Start-to-End Run**: Begin at `Pages/main.html`, accept a job, comply with the payroll ID check, and purchase at least one hobby to verify that ongoing effects render in the HUD.
...
- **Mobile Gate**: Narrow the viewport, observe `Pages/mobile.html`, then widen and use the provided button to restore the game iframe.

## Maintenance Checklist
- When adding a new scenario, register it in `buildScenarioLibrary`, create a corresponding stub in `Pages/`, and decide whether it belongs to the `core` or `random` pool.
...
- `styles.css`
- `scripts.js`
- `Pages/` (31 HTML files; scenario stubs plus intro, learn-more, mobile, no-happiness, and identity theft screens)
